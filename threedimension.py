...
class ThreeD(
    ...
    def __init__(self, number_of_points, number_of_lines):
        ...
        for i in range(number_of_lines):
            ...
            self.traces[i] = gl.GLLinePlotItem(
                pos=specanpts,
                color=np.zeros([self.number_of_points, 4]),
                antialias=False,
                mode="line_strip",
            )
        ...
        for i in range(self.number_of_lines):
            ...
            specanpts = np.vstack([self.x, self.y, self.z]).transpose()
            self.traces[i] = gl.GLLinePlotItem(
                pos=specanpts, color=[0, 1, 0, 1], antialias=False, mode="line_strip"
            )
            self.widget.addItem(self.traces[i])
    ...
    def update(self):
        ...
        for i in range(self.number_of_lines - 1):
            ...
            oldlinepoints[:, 1] = self.lineyvalues[
                i
            ]  # set y value of previous line to increment
            self.set_plotdata(
                name=i, points=oldlinepoints, color=oldlinecolours, width=1
            )  # plot previous line
        ...
        specanpts = np.vstack([self.x, self.y, self.z]).transpose()
        self.set_plotdata(
            name=self.number_of_lines - 1, points=specanpts, color=goodcolours, width=1
        )
