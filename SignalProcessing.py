...
class process:
    ...
    def lowpass_filter(self, data, cutoff, fs, order=5):
        b, a = self.butter_lowpass(cutoff, fs, order=order)   # This self bit might be wrong
        y = filtfilt(b, a, data)
        return y
    
    # Window
    # Generate window for multiplication with samples before calculating FFT.
    ...
    # Phase Information:
    # Extract phase information from the FFT results.
    def get_phase(self, fft):
        return np.angle(fft)

    # Power Spectral Density
    # Estimate the power spectral density to analyse the power distribution of the signal across frequencies.
    ...
    # Normalizs the samples or FFT results to a specific range or scale.
