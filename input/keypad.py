...
class Keypad:
    ...
    def bind_ui(self):
        ...
        self.button_ghz.setToolTip("Shortcut key: G")
        self.button_mhz.setToolTip("Shortcut key: M")
        self.button_khz.setToolTip("Shortcut key: K")
    ...
    def on_frequency_select_inner(self, multiplier: int):
        ...
        def on_frequency_select():
            # In preset_name mode any unit key confirms the name
            if getattr(self.ui, 'frequency_entry_mode', None) == 'preset_name':
                pm = getattr(self.ui, 'preset_manager', None)
                if pm:
                    pm.confirm_name(self.ui.preset_name_slot, self.ui.preset_name_text)
                return
            try:
