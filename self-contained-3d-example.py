...
class Visualizer():
    ...
    def __init__(self):
        ...
        griditem.rotate(90, 1, 0, 0)  # This effectively positions it at Z = 0
        griditem.translate(0,10,0)
        self.gldisplay.addItem(griditem)
                
        centre_text = gl.GLTextItem()
        centre_text.setData(pos=(0.0, 10.0, 10.0), color=(255, 255, 255, 255), text='Centre frequency')
        self.gldisplay.addItem(centre_text)
        # Print the text and font details
        print(f"POS:    {centre_text.pos}")
        print(f"Colour: {centre_text.color}")
        print(f"Text:   {centre_text.text}")  
        print(f"Font:   {centre_text.font}")

        line_points = [(0, 10, 10), (0, 10, 0)]  # Define the start and end points
        line = gl.GLLinePlotItem(pos=line_points, color=(1, 1, 1, 1), width=2)
        self.gldisplay.addItem(line)
    ...
    def update(self):
        if self.pause!=('enabled'):
            ...
            new_peak_text=(str(np.max(self.z)))
            self.peak_text.setData(pos=(10.0, 10.0, 10.0), color=(255, 255, 255, 255), text=new_peak_text)
